# Build Real Atmospheres and deploy it to the game's user-writable mods folder (dev loop).
# No elevation needed: everything lives under Documents\My Games\Kitten Space Agency.
...
$proj = $PSScriptRoot
dotnet build "$proj\RealAtmospheres.csproj" -c Release
...
$dest = Join-Path $gameDir "mods\RealAtmospheres"
...
Copy-Item "$proj\bin\Release\RealAtmospheres.dll" $dest -Force
...
if ((Get-Content $manifest -Raw) -notmatch 'id = "RealAtmospheres"') {
    Add-Content $manifest "`n[[mods]]`nid = `"RealAtmospheres`"`nenabled = true" -Encoding utf8
    Write-Host "added RealAtmospheres to $manifest"
}
